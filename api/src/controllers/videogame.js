...
//   } else {
    ...
    try{
      ...
      const {data} = await axios.get(`${GAMES_URL}${process.API_KEY}`);
...
async function getVideogameId(req, res, next) {
  ...
  try {
    if (validator.isUUID(id)) {
      ...
      const { data } = await axios.get(`${GAMES_URL}/${id}${process.API_KEY}`)
      return res.json({
        id: data.id,
        name: data.name,
        ...
        platforms: data.platforms.map(platform => ({id: platform.platform.id, name: platform.platform.name})),
